...
def binary_search(down: str, hi: int):
    def go(lo: int, hi: int, s: str):
        if lo == hi or not s:
            return lo
        ...
        offset = (hi - lo) // 2
        if head == down:
            return go(lo, hi - offset, tail)
        return go(lo + offset, hi, tail)

    return lambda s: go(0, hi, s)


def parse_boarding_pass(pass_spec: str) -> Seat:
    return Seat(
        row=binary_search("F", 128)(pass_spec[:7]),
        col=binary_search("L", 8)(pass_spec[7:]),
    )
...
def test_part1():
    ...
    for spec, expected, expected_id in tests:
        assert (got := parse_boarding_pass(spec)) == expected
